...
This document outlines requirements for an Android application designed to assist a person in hunting and recovering weather balloons. The app's design is centered around a single-screen, map-based interface that provides all critical information in real-time as they pursue a balloon.
...
│  (MySondyGo Device) │    │  (SondeHub API)     │    │  (Fused Location Provider)    │
└─────────────────────┘    └─────────────────────┘    └─────────────────────┘
          │                          │                          │
...
│    RadioChannelData │    │    RadioChannelData │    │   distance (meters)│
│    • frequency      │    │    • frequency      │    │                     │
...
│ • Smart caching     │ │ • Source tracking   │ │ • Google Maps API    │
...
Services StateFlow → MapViewModel StateFlow → Compose UI collects via collectAsState() (no middleman)

🔄 **Service Coordination:**
ServiceCoordinator handles app infrastructure only, UI state handled by MapViewModel
...
Architecture enforces a clean separation of responsibilities while embracing a modular service layout, a coordinator, and a map presenter.
  ...
  - Business logic lives in services and the coordinator; Jetpack Compose views remain declarative, consuming ready-to-render state from environment objects without reaching into data sources directly.
  ...
  - Presenter Layer for Complex Views
    The MapViewModel owns map-specific state transformations (overlay generation, formatted strings, user intents) so TrackingMapView does not need to manipulate coordinator or services state directly. This keeps the coordinator focused on orchestration and the view focused on layout.
  - Kotlin Flow-Driven Data Flow
    Services publish changes via StateFlow/SharedFlow properties; the coordinator and presenter subscribe, transform, and re-publish derived values. This provides a single reactive pipeline from BLE/APRS/location inputs to UI outputs.
  - Environment-Driven UI: Views observe the coordinator, presenter, and settings objects through @collectAsState. User actions (toggle heading, trigger prediction, open Maps) bubble up through intent methods defined on the presenter/coordinator instead of mutating state locally.
  - Persistence & Caching: PersistenceService handles simple file-based persistence (current track, landing history, user settings) using a single-sonde snapshot model. Caches prevent redundant prediction/route work. Both are injected once through AppServices, reinforcing a single source of truth.
...
val position by balloonPositionService.positionState.collectAsState()
...
serviceCoordinator.openInGoogleMaps() // Uses location + routing + settings

// Coordinated: multi-service state
val consolidatedFlightData: StateFlow<FlightData> // BLE + APRS + predictions
```
...
| Map camera updates | Direct via MapViewModel | Map-specific coordination |
...
#### Android Module/Layout Overview

The codebase is organized into Android-oriented layers with clear separation of responsibilities:

**Infrastructure Layer:**
- `di/` - Hilt modules (AppModule, ServiceModule)
- `core/models/` - Shared data classes, enums, logging utilities
- `core/persistence/` - DataStore/Room/JSON persistence and file storage

**Communication Layer:**
- `ble/` - MySondyGo BLE protocol implementation and device communication
- `aprs/` - SondeHub API integration and APRS data management
- `location/` - Fused location tracking and proximity detection

**Processing Layer:**
- `state/` - Telemetry state machine, position tracking, and motion analysis
- `prediction/` - Tawhiri API integration with prediction cache
- `routing/` - Google Directions integration with RoutingCache

**Coordination Layer:**
- `coordinator/` - ServiceCoordinator orchestration and cross-service state management
- `startup/` - Startup sequence and multi-service initialization
- `map/` - MapViewModel and map-specific transformations

**UI Layer (Compose):**
- `ui/map/` - TrackingMapScreen and map overlays
- `ui/panel/` - DataPanel composables
- `ui/settings/` - Settings screens

**Utility Layer:**
- `settings/` - UserSettings and DeviceSettings models
- `logging/` - CSV telemetry logging and debug tooling
...
Single-activity Compose app. `MainActivity` initializes Hilt, creates the `ServiceCoordinator`, and injects shared services and ViewModels into Compose. Lifecycle hooks manage persistence saves and notification routing.

#### Dependency Injection

Use Hilt to wire the core infrastructure: `PersistenceService`, `BLECommunicationService`, `CurrentLocationService`, `BalloonPositionService`, `BalloonTrackService`, `LandingPointTrackingService`, caches, and other singletons.

#### ServiceCoordinator

Central coordinator that manages service activation, arbitrates telemetry between BLE and APRS, and provides cross-service operations (routing, predictions, landing updates).

#### Startup Sequence

Encapsulate the 8-step startup sequence in a dedicated startup component that can run in parallel where possible (BLE scan, APRS priming, map readiness).

#### Routing Service

**Google Directions Integration with Performance Optimization:**

Comprehensive routing service with caching and proximity-aware behavior:

**Cache Strategy:**
```kotlin
private fun generateCacheKey(start: LatLng, end: LatLng, mode: TransportationMode): String {
    val startKey = "${round(start.latitude * 100) / 100}-${round(start.longitude * 100) / 100}"
    val endKey = "${round(end.latitude * 100) / 100}-${round(end.longitude * 100) / 100}"
    return "route-$startKey-$endKey-${mode.name}"
}
```

**Proximity Rules:**
- Route visibility hidden when user within 200 m of landed balloon
- Recalculation triggers: user movement >100 m, transport mode change, landing point updates
- Cache prevents redundant Directions API calls
...
**Advanced Features:**
- Bike mode ETA scaling (multiply by 0.7) when real bicycle routes are unavailable
- Straight-line fallback ensures UI always has a path to display
- TTL-based expiration, LRU eviction, coordinate quantization
- Directions API may return no bike route; fall back gracefully

**Integration Points:**
- Google Maps hand-off with transport mode preservation
- Map overlay: green route line with real-time updates
- Data panel: ETA and distance calculations
- Notification system: landing point change alerts

#### Map ViewModel

**Hybrid Architecture Pattern**: MapViewModel uses both direct service access and ServiceCoordinator integration:

- Direct access: map-specific data (balloon position, track points, landing points)
- Coordinated access: cross-service operations (prediction paths, user routes)
- Map-specific coordination: aggregates, transforms, and republishes map state

**Implementation Pattern:**
```kotlin
class MapViewModel(
    private val balloonPositionService: BalloonPositionService,
    private val balloonTrackService: BalloonTrackService,
    private val coordinator: ServiceCoordinator,
) : ViewModel() {
    val annotations: StateFlow<List<MapAnnotationItem>> = MutableStateFlow(emptyList())
    val cameraUpdate: StateFlow<CameraUpdate?> = MutableStateFlow(null)
}
```

**Responsibilities:**
- Map annotation generation and management
- Camera position and zoom coordination
- Map overlay state transformation (track lines, prediction paths, routes)
- User intent handling (toggle heading mode, trigger predictions, open Google Maps)
- Distance calculations and formatting for map display

#### Tracking Map UI

Compose-based map screen with the control strip, map view (70% height), map type toggle, and distance overlay. Uses Google Maps Compose or MapView interoperability to render overlays.

#### Data Panel UI

Compose-based data panel with the two-table layout and live telemetry display.

#### Settings UI

Compose settings screens for sonde settings, prediction settings, tune, and device settings tabs.

#### Prediction Service

Implements Tawhiri API integration with cache and error handling. Uses coroutines, structured concurrency, and cancellation to avoid stale results after sonde changes.

#### Debug CSV Logger

Records telemetry frames (excluding development sondes) to a CSV file in the app external files directory for offline analysis.
...
1. Constructed with a `PersistenceService` reference and `BluetoothManager`.
2. Retrieves `BluetoothAdapter`, creates a `BluetoothLeScanner`, and registers an adapter state receiver.
...
- `latestTelemetry`, `deviceSettings`, `connectionStatus`, `isReadyForCommands` as StateFlow.
- `telemetryData` as a SharedFlow/Channel for telemetry frames.
- Internal state: `lastMessageType`, `lastTelemetryUpdateTime`, `isBLETelemetryStale` (not exposed).

**Bluetooth Adapter Lifecycle**

- When Bluetooth is ON, start scanning.
- When Bluetooth is OFF/unauthorized, set `connectionStatus = notConnected`, clear GATT, and stop scanning.

**Scan and Connection**

- Scan for devices advertising the UART service UUID; accept devices whose name contains "MySondy".
- On discovery, stop scanning and connect via `connectGatt` (autoConnect = false).
- On disconnect, set `connectionStatus = notConnected` and restart scanning.

**GATT Service & Characteristic Discovery**

- On connection, call `discoverServices()` and find UART RX/TX characteristics.
- Enable notifications on RX via `setCharacteristicNotification` and CCCD write.
- Choose TX write characteristic preferring WRITE, falling back to WRITE_NO_RESPONSE.

**Receiving Data**

- `onCharacteristicChanged` receives bytes from RX, decodes UTF-8, and passes the string to `parseMessage(_)`.
...
- All Android Bluetooth LE errors are caught and logged without crashing the service.
...
**Purpose**  
Keep track of the Android phone’s location and heading, swap automatically between low-power background tracking and precise “heading mode,” detect meaningful movement, and publish the raw distance to the balloon so the UI can render the overlay.

#### When it runs

- Whenever the user changes location permissions.
- Whenever Android delivers new location updates (background mode roughly every 30 s, precision mode every few meters/seconds).
- Whenever the compass heading changes.
- Whenever the presenter/coordinator provides an updated balloon position.

#### What it listens to

- Location and heading events emitted by Fused Location Provider.
...
- Uses FusedLocationProviderClient with two LocationRequest profiles: background (coarse, 10 m / ~30 s) and heading (high accuracy, 1-2 s / small displacement).
- Foreground-only updates; background location updates are disabled when the app is backgrounded (per overall background policy).
- `enableHeadingMode()` / `disableHeadingMode()` switch between request profiles and log the change for diagnostics.
- Each location callback builds a new `LocationData`, throttles precision updates to at least 1 s apart, logs movement jumps (>20 m), updates the significant-movement marker, and recomputes both distance-to-balloon and the 200 m proximity flag.
- Heading is taken from the device bearing (location bearing) when available; if missing, fall back to sensor-based heading.
- `updateDistanceToBalloon()` computes the raw meters from user to balloon; formatting (e.g., "123 m" vs "1.2 km") is handled by the UI.
- `updateBalloonDisplayPosition(_:)` is called whenever the MapViewModel pushes a new balloon coordinate, so distance and proximity are refreshed immediately.
- Errors or denials are logged via `appLog`; routine health spam is muted. Location requests are removed when the service is stopped.
...
1. StateFlow var userSettings: UserSettings - Prediction parameters
...
- `trackUpdated` (Kotlin Flow subject), `landingPosition`, `balloonPhase`.
...
2. On each trigger:
   ...
   - Runs a coroutine-based HTTP request (OkHttp/Retrofit) with a 30 s timeout; interprets the JSON response into `PredictionData` (path, burst point, landing point/time). See [Prediction Service API (Sondehub)](#prediction-service-api-sondehub) for request/response details.
...
6. **Task cancellation**: Tracks `currentPredictionTask` and cancels it immediately when starting a new prediction or when `clearAllData()` is called during sonde changes. Running tasks check `ensureActive()` before cache lookup, API call, and result publishing to exit cleanly with `cancellation` when cancelled. This prevents stale results from old sondes being published after sonde change completes.

#### Notes

- Predictions are skipped entirely if no telemetry is available or the balloon is already marked as landed.
- The service lives on the Main dispatcher so published state updates remain synchronous with the UI.
- CSV logging and other long-term diagnostics remain outside this service to keep it focused on API orchestration.

### Route Calculation Service

**Purpose**  
Request a route from the user’s current location to the landing point using Google Maps when possible, fall back to heuristics when it is not, and avoid redundant work via an in-memory cache.

#### Inputs

- User location (`LocationData`).
- Landing point coordinate (`LatLng`).
...
- `RoutingCache` stores recently computed routes keyed by origin/destination/mode.

#### Behavior

1. Logs each request, then builds a Directions API request from the user location to the landing point using the preferred transport type (car -> driving, bike -> bicycling when available).
2. Attempts to calculate the route via the Directions API.
3. If a route is returned, converts the polyline to `[LatLng]`, adjusts the ETA for bike mode (multiplies by 0.7 when using non-bicycle fallbacks), and wraps everything in `RouteData`.
4. If the Directions API returns no route:
   - Tries up to ten random 500 m offsets around the destination.
   - Runs a radial search (300/600/1200 m every 45 degrees) while no route is available.
   - If nothing succeeds, falls back to a straight-line segment with a heuristic speed (car 22.0 m/s, bike 4.2 m/s) to produce distance and arrival estimate.
5. Any other error is propagated to the caller so the UI can handle failure states (e.g., hide the overlay).
6. Successful routes are cached in `RoutingCache` for 5 minutes (LRU eviction, capacity 100). Callers check the cache before invoking Google Maps again.
7. **Task cancellation**: Tracks the current route calculation coroutine `Job` and cancels it immediately when starting a new route calculation or when `clearAllData()` is called during sonde changes. Running coroutines check `ensureActive()` before route calculation and before publishing results to exit cleanly when cancelled. This prevents stale routes to old sonde landing points being published after sonde change completes.
...
- Bike travel times are shortened by 30% to offset conservative walking-based estimates when bicycle routes are unavailable; Google Maps hand-off still requests bicycling when supported.
- Straight-line fallback ensures the UI always has a path to display, even without Google Maps coverage.
- `RoutingCache` entries expire automatically; metrics are logged for cache hits/misses.

### Navigation Service

**Purpose**
Manage external navigation integration with Google Maps and provide landing point change notifications for users navigating with Android Auto or external navigation apps.

#### Inputs

- Landing point coordinates for Google Maps launch
- New landing point coordinates for change detection

#### Publishes

- None (triggers external actions: Google Maps launch, Android notifications)

#### Behavior

**Google Maps Integration**:
1. Receives landing point coordinate from ServiceCoordinator.
2. Builds an Android intent for Google Maps navigation with the selected transport mode.
   - Car mode -> driving directions
   - Bike mode -> bicycling when available, fallback to walking
3. Launches Google Maps with the destination pre-filled.
...
2. When landing point changes:
   ...
   - If movement >300 m -> sends Android notification.
...
3. Notification includes:
   ...
   - Destination coordinates for tap handling
...
**Android Auto Use Case**:
- BalloonHunter runs in foreground on the Android phone screen.
- Google Maps displays on Android Auto screen (separate process).
- Notifications alert user to significant landing point changes while driving.
- User can glance at the phone to see updated landing prediction.

#### Notes

- Notifications only fire when app is in foreground (background predictions suspended per BACKGROUND_TRACKING.md).
- 300 m threshold prevents spurious alerts from minor prediction adjustments.
...
- User intents coming from the presenter/views (toggle heading, mute buzzer, open settings, launch Google Maps, etc.).
...
5. **Route management** — Requests routes when the landing point or user location changes, updates the green overlay, surfaces ETA/distance in the data panel, and honours cache hits to avoid redundant Google Maps calls.
...
7. **Google Maps hand-off & navigation notifications** - `openInGoogleMaps()` launches navigation using the selected transport mode (car -> driving, bike -> bicycling when available with walking fallback). Landing point updates automatically trigger NavigationService to check for significant changes (>300m) and send Android notifications to alert users during Android Auto navigation.
...
**Async operation cancellation:** Services with long-running async operations (predictions, route calculations, APRS fetches) track their current task using Kotlin coroutines. When `clearAllData()` is called, each service immediately cancels its in-flight task via `job.cancel()` without waiting for completion. The running task checks for cancellation at key points using `ensureActive()` and exits cleanly with `cancellation`. This prevents stale results from old sondes being published after the sonde change completes. Cancellation is instantaneous - the coordinator doesn't wait for tasks to finish, they clean up asynchronously.
...
* Google Maps navigation: Opens Google Maps with the current landing point pre-filled. Car mode launches driving directions; bicycle mode launches bicycling directions when available and falls back to walking otherwise.  

* **Settings Button**: Gear icon that opens device configuration interface and requests current device parameters via BLE.

* **Transport Mode Picker**: Segmented control with car/bicycle icons for selecting navigation mode (affects routing calculations and Google Maps integration).
...
* **Heading Mode Toggle**: Location icon button that toggles between camera positions:
  - **"Heading" mode**: Map centers on Android phone position and aligns to device heading (zoom-only interaction)
...
* **Google Maps Button**: Navigation icon (only visible when landing point available) that opens Google Maps with pre-configured directions using selected transport mode.
...
* User Position: The Android phone's current position is continuously displayed and updated on the map, represented by a runner.  
...
* Balloon Predicted Path: The complete predicted flight path from the Sondehub API is displayed as a thick bright cyan-blue line (8dp width, color #00AAFF). While the balloon is ascending, a distinct marker indicates the predicted burst location. The burst location shall only be visible when the balloon is ascending. The display of this path is controlled by the button “Prediction on/off”. The path should appear on the map when it is available.  
* Landing point: Shall be visible if a valid landing point is available  
* Planned Route from User to Landing Point: The recommended route from the user's current location to the predicted landing point is retrieved from Google Maps and displayed on the map as a green overlay path. When bicycle mode is selected the in-app overlay still uses walking directions (Google Maps SDK limitation) but the Google Maps button launches native cycling navigation. The route should appear on the map when it is available.  
...
It is not shown if the distance between the balloon and the Android phone is less than 100m.
...
This panel covers the full width of the display and the full height left by the map. It displays the following fields (font sizes adjusted to ensure all data fits within the view without a title):
  ...
  * Compose compatibility: avoid deprecated lifecycle and state APIs
...
The app's data flow is designed to be reactive, with Jetpack Compose views updating automatically as new data becomes available from the various services.
...
2.  **Service Processing**: These services, along with `BalloonTrackService` and `PredictionService`, process the raw data, calculate derived values (e.g., smoothed speeds, predictions), and publish the results via `StateFlow` properties.

3.  **View Consumption**: Jetpack Compose views, such as `DataPanelView` and `TrackingMapView`, use the `@collectAsState` property wrapper to subscribe directly to the services they need. They access the `StateFlow` properties of these services to get the data they need for display.
...
While the `MapViewModel` handles complex presentation logic for the map, simpler views like `DataPanelView` consume data directly from the services for a more direct and efficient data flow. Some minor formatting logic may reside within the views for simplicity.
...
(These values are stored permanently on the Android phone via PersistenceService and are never transmitted to the device)
